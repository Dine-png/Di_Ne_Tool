...
namespace DiNeTool.InGameChecker
{
    public static class DiNeAvatarStats
    {
        ...
        public static StatsData Calculate(GameObject avatarRoot)
        {
            ...
            {
                data.MaterialSlotCount += r.sharedMaterials != null ? r.sharedMaterials.Length : 0;
            ...
            foreach (var tex in uniqueTextures)
                data.VRAMBytes += DiNeTextureVRAM.CalcTextureVRAM(tex);
            ...
            foreach (var kvp in meshTriLengthCache)
            {
                ...
                long meshBytes = indices + vertices;
                data.MeshMemoryBytes += meshBytes;
                data.UploadSizeBytes += meshBytes;
            }
